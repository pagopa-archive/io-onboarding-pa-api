...
>NOTE: the *docker-compose.yaml* file sets some environment variables that could be used to adapt the application features to specific needs. The *docker-compose.yaml* file sources variables from a *.env* file that should be placed in the root folder of this repository. An example file *.env.example* is distributed with the code. For your convenience, you may start copying this file, renaming it to *.env* and editing its values. More info about variables can be found in the dedicated paragraph, below in the readme.
